...
class DataProvider(object):
    ...
    def data_name_to_seg_name(self, data_filename):
        return data_filename.replace('Case', 'mask_case')

    def ESC_transferred(self, data_filename):
        return data_filename.replace('\\', '/')

    def get_patch_seg_from_data(self, data_fullpath, seg_patch_dir):
        seg_filename = self.data_name_to_seg_name(data_fullpath.split('/')[-1])
    ...
    def read_volume(self, volume_path, with_seg=False):
        ...
        if with_seg:
            seg_volume = nib.load(self.ESC_transferred(self.get_volume_seg_from_data(volume_path, self.seg_volume_dir))).get_data()
